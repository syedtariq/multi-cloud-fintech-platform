...
resource "aws_dms_replication_instance" "cross_cloud" {
  ...
  replication_instance_id     = "${var.name_prefix}-cross-cloud-replication"
  replication_subnet_group_id = aws_dms_replication_subnet_group.cross_cloud.id
  vpc_security_group_ids      = [aws_security_group.dms.id]

  tags = merge(var.common_tags, {
    Name = "${var.name_prefix}-dms-replication"
  })
}
...
resource "aws_dms_replication_subnet_group" "cross_cloud" {
  ...
  tags = merge(var.common_tags, {
    Name = "${var.name_prefix}-dms-subnet-group"
  })
}
...
resource "aws_vpn_gateway" "cross_cloud" {
  ...
  tags = merge(var.common_tags, {
    Name = "${var.name_prefix}-cross-cloud-vpn-gw"
  })
}
...
resource "aws_customer_gateway" "azure" {
  ...
  tags = merge(var.common_tags, {
    Name = "${var.name_prefix}-azure-customer-gw"
  })
}
...
resource "aws_vpn_connection" "azure" {
  ...
  tags = merge(var.common_tags, {
    Name = "${var.name_prefix}-azure-vpn-connection"
  })
}
...
resource "aws_security_group" "dms" {
  name_prefix = "${var.name_prefix}-dms-"
  vpc_id      = var.vpc_id
...
resource "aws_dms_endpoint" "source" {
  ...
  port            = 5432
  server_name     = var.rds_cluster_endpoint
  ssl_mode        = "require"
  username        = "postgres"

  tags = merge(var.common_tags, {
    Name = "${var.name_prefix}-source-endpoint"
  })
}
...
resource "aws_dms_endpoint" "target" {
  ...
  tags = merge(var.common_tags, {
    Name = "${var.name_prefix}-target-endpoint"
  })
}
...
resource "aws_dms_replication_task" "cross_cloud" {
  ...
  replication_task_id      = "${var.name_prefix}-cross-cloud-task"
  ...
  tags = merge(var.common_tags, {
    Name = "${var.name_prefix}-cross-cloud-replication-task"
  })
}
...
resource "aws_lambda_function" "redis_replication" {
  ...
  function_name    = "${var.name_prefix}-redis-replication"
  ...
  environment {
    variables = {
      AWS_REDIS_ENDPOINT    = var.redis_primary_endpoint
  ...
  tags = merge(var.common_tags, {
    Name = "${var.name_prefix}-redis-replication"
  })
}
...
resource "aws_iam_role" "lambda_redis_replication" {
  ...
  name = "${var.name_prefix}-lambda-redis-replication"
...
resource "aws_iam_role_policy" "lambda_redis_replication" {
  ...
  name = "${var.name_prefix}-lambda-redis-policy"
...
resource "aws_cloudwatch_event_rule" "redis_replication" {
  ...
  name                = "${var.name_prefix}-redis-replication-schedule"
